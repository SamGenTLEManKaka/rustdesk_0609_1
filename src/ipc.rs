...
pub enum Data {
    ...
    OnlineRelayStatus(bool),
    OnlineConnStatus(bool),
...
async fn handle(data: Data, stream: &mut Connection) {
    match data {
        ...
        Data::OnlineRelayStatus(_) => {
            let x = config::ONLINE_RELAY
                .lock()
                .unwrap()
                .clone();
            allow_err!(stream.send(&Data::OnlineRelayStatus(x)).await);
        }
        Data::OnlineConnStatus(_) => {
            let z = config::ONLINE_CONN
                .lock()
                .unwrap()
                .clone();
            allow_err!(stream.send(&Data::OnlineConnStatus(z)).await);
        }
