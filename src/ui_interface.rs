...
type Status = (i32, bool, i64, String,bool,bool); // (status_num, key_confirmed, mouse_time, id,realy_status)

lazy_static::lazy_static! {
    static ref UI_STATUS : Arc<Mutex<Status>> = Arc::new(Mutex::new((0, false, 0, "".to_owned(),false,false)));
...
async fn check_connect_status_(reconnect: bool, rx: mpsc::UnboundedReceiver<ipc::Data>) {
    ...
    loop {
            ...
            loop {
                tokio::select! {
                    res = c.next() => {
                        match res {
                            ...
                            Ok(Some(ipc::Data::OnlineStatus(Some((mut x, c))))) => {
                                ...
                                UI_STATUS.lock().unwrap().0 = x as _;
                                UI_STATUS.lock().unwrap().1 = key_confirmed;
                                UI_STATUS.lock().unwrap().2 = mouse_time;
                                UI_STATUS.lock().unwrap().3 = id.clone();
                            }
                            Ok(Some(ipc::Data::OnlineRelayStatus(z))) => {
                                UI_STATUS.lock().unwrap().4 = z;
                            }
                            Ok(Some(ipc::Data::OnlineConnStatus(w))) => {
                                UI_STATUS.lock().unwrap().5 = w;
                            }
                    ...
                    _ = timer.tick() => {
                        ...
                        c.send(&ipc::Data::OnlineRelayStatus(false)).await.ok();
                        c.send(&ipc::Data::OnlineConnStatus(false)).await.ok();
        ...
        *UI_STATUS.lock().unwrap() = (-1, key_confirmed, mouse_time, id.clone(),false,false);
        sleep(1.).await;
    }
}
