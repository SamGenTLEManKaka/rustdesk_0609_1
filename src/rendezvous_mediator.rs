...
use hbb_common::{
    allow_err,
    anyhow::bail,
    config::{Config, REG_INTERVAL, RENDEZVOUS_PORT, RENDEZVOUS_TIMEOUT,RELAY_PORT},
...
impl RendezvousMediator {
    ...
    pub async fn start(server: ServerPtr, host: String) -> ResultType<()> {
        ...
        // let host_realy = crate::check_port(Config::get_relay_server(), RELAY_PORT);
        ...
        loop {
            let mut update_latency = || {
                ...
                log::info!("latency =  {} , old_latency = {} ,n = {} ,(latency - old_latency).abs() = {}", latency, old_latency,n,(latency - old_latency).abs());
            ...
            select! {
                ...
                _ = timer.tick() => {
                    ...
                    let host_realy = crate::check_port(Config::get_relay_server().first().unwrap(), RELAY_PORT);
                    if let Ok(_s) = tokio::time::timeout(Duration::from_secs(1), tokio::net::TcpStream::connect(&host_realy)).await {
                        Config::update_relay_status(true);
                    } else {
                        Config::update_relay_status(false);
                    }
                    if let Ok(_w) = tokio::time::timeout(Duration::from_secs(1), tokio::net::TcpStream::connect(&host)).await {
                        Config::update_conn_status(true);
                    } else {
                        Config::update_conn_status(false);
                    }
                }
            }
        }
