...
lazy_static::lazy_static! {
    ...
    pub static ref ONLINE_RELAY: Arc<Mutex<bool>> = Default::default();
    pub static ref ONLINE_CONN: Arc<Mutex<bool>> = Default::default();
    pub static ref REALY_SERVER: Arc<Mutex<String>> = Default::default();
    pub static ref PROD_RENDEZVOUS_SERVER: Arc<RwLock<String>> = Arc::new(RwLock::new(match option_env!("RENDEZVOUS_SERVER") {
        Some(key) if !key.is_empty() => key,
        _ => "",
    }.to_owned()));
    pub static ref APP_NAME: Arc<RwLock<String>> = Arc::new(RwLock::new("汇川内网远程工具 v1.0.1".to_owned()));
...
impl Config {
    ...
    pub fn get_relay_server() -> Vec<String> {
        let s = Self::get_option("relay-server");
        return vec![s];
    }
    ...
    pub fn update_relay_status(status: bool) {
        *ONLINE_RELAY.lock().unwrap() = status;
    }
    pub fn update_conn_status(status: bool) {
        *ONLINE_CONN.lock().unwrap() = status;
    }
